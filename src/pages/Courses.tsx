...
const Courses = () => {
  ...
  // Fallback categories to show when API fails
  const fallbackCategories = [
    { value: 'all', label: isRTL ? 'جميع الفئات' : 'All Categories' },
    { value: 'web-dev', label: isRTL ? 'تطوير الويب' : 'Web Development' },
    ...
    const fetchData = async () => {
      ...
      try {
          ...
          supabase
            ...
            .select(`
              *,
              ...
              categories!courses_category_id_fkey ( name_en, name_ar )
            `)
        ...
        if (categoriesData && categoriesData.length > 0) {
          setCategories([
            { value: 'all', label: isRTL ? 'جميع الفئات' : 'All Categories' },
            ...categoriesData.map((c: any) => ({ value: c.id, label: isRTL ? c.name_ar : c.name_en }))
          ]);
        } else {
        ...
        // If no courses returned, use fallback data
  ...
  const handleRetry = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (categoriesError) {
          console.error('Supabase error fetching categories on retry:', categoriesError);
          throw categoriesError;
        }
        ...
        const coursesWithStats = await Promise.all((coursesData || []).map(async (course: any) => {
          try {
            ...
            console.warn(`Error fetching stats for course ${course.id} on retry:`, statsError);
            return { ...course, lessons_count: 0, students_count: 0, average_rating: 0 };
          }
        }));
        ...
        console.error('Error loading courses on retry', e);
        setError(e.message || 'Failed to load courses');
        setCategories(fallbackCategories);
        setCourses(fallbackCourses);
      } finally {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center space-y-4 mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              {isRTL 
                ? 'اكتشف مجموعة واسعة من الدورات التعليمية عالية الجودة'
                : 'Discover a wide range of high-quality educational courses'
              }
            </p>
            ...
            {error && (
              <div className="flex items-center justify-center gap-3 p-3 bg-destructive/10 border border-destructive/20 rounded-lg max-w-md mx-auto">
                ...
                <span className="text-sm text-destructive">
                  {isRTL ? 'حدث خطأ في تحميل الدورات' : 'Error loading courses'}
                </span>
                <Button 
                  variant="outline" 
                  size="sm" 
          ...
          <div className={`grid gap-6 ${
            ...
            {filteredCourses.map((course) => (
                <CourseCard 
                  ...
                  description={course.short_description_en}
                  ...
                  category={course.categories.name_en}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {isRTL ? 'لم يتم العثور على دورات' : 'No courses found'}
              </h3>
