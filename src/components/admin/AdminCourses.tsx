...
interface Course {
  ...
  preview_video_url: string | null; // Added
  price: number;
  ...
  requirements_en: string | null; // Added
  requirements_ar: string | null; // Added
  syllabus_en: string | null;     // Added
  syllabus_ar: string | null;     // Added
  created_at: string;
  updated_at: string;
...
const AdminCourses = () => {
  ...
  const [courseForm, setCourseForm] = useState<Partial<Course>>({
    ...
    requirements_en: '',          // Added
    requirements_ar: '',          // Added
    syllabus_en: '',              // Added
    syllabus_ar: ''               // Added
  });
  ...
  const handleCreateCourse = async () => {
    try {
      ...
      const { error } = await supabase
        .from('courses')
        .insert([courseForm as unknown as Course]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto pr-2">
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'الوصف الكامل (EN)' : 'Description (EN)'}
        </label>
        <Textarea
          value={courseForm.description_en || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, description_en: e.target.value })
          }
          placeholder={isRTL ? 'أدخل الوصف الكامل بالإنجليزية' : 'Enter full description in English'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'الوصف الكامل (AR)' : 'Description (AR)'}
        </label>
        <Textarea
          value={courseForm.description_ar || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, description_ar: e.target.value })
          }
          placeholder={isRTL ? 'أدخل الوصف الكامل بالعربية' : 'Enter full description in Arabic'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'المتطلبات (EN)' : 'Requirements (EN)'}
        </label>
        <Textarea
          value={courseForm.requirements_en || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, requirements_en: e.target.value })
          }
          placeholder={isRTL ? 'أدخل المتطلبات بالإنجليزية' : 'Enter requirements in English'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'المتطلبات (AR)' : 'Requirements (AR)'}
        </label>
        <Textarea
          value={courseForm.requirements_ar || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, requirements_ar: e.target.value })
          }
          placeholder={isRTL ? 'أدخل المتطلبات بالعربية' : 'Enter requirements in Arabic'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'محتوى الدورة (EN)' : 'Syllabus (EN)'}
        </label>
        <Textarea
          value={courseForm.syllabus_en || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, syllabus_en: e.target.value })
          }
          placeholder={isRTL ? 'أدخل محتوى الدورة بالإنجليزية' : 'Enter syllabus in English'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'محتوى الدورة (AR)' : 'Syllabus (AR)'}
        </label>
        <Textarea
          value={courseForm.syllabus_ar || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, syllabus_ar: e.target.value })
          }
          placeholder={isRTL ? 'أدخل محتوى الدورة بالعربية' : 'Enter syllabus in Arabic'}
        />
      </div>
      ...
      <div className="md:col-span-2">
        <label className="text-sm font-medium">
          {isRTL ? 'رابط فيديو العرض' : 'Preview Video URL'}
        </label>
        <Input
          value={courseForm.preview_video_url || ''}
          onChange={(e) =>
            setCourseForm({ ...courseForm, preview_video_url: e.target.value })
          }
          placeholder={isRTL ? 'أدخل رابط فيديو العرض' : 'Enter preview video URL'}
        />
      </div>
